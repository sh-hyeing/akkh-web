  ...
  <>
   ...
   >
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "5px", marginBottom: "6px" }}>
     ...
     <Rose size={18} opacity={0.65} />
     <Bud size={14} opacity={0.6} />
     <Rose size={22} opacity={0.72} />
     <Bud size={14} opacity={0.6} />
     <Rose size={18} opacity={0.65} />
     <Bud size={12} opacity={0.55} />
    </div>
     ...
     {["main", "notice", "sample", "guest"].map((menu) => (
      <button
       ...
       style={{
        ...
        background: "none",
        border: "none",
        color: currentMenu === menu ? "#8c7260" : "#c8bfb0",
        borderBottom: currentMenu === menu ? "1.5px solid #c09070" : "1.5px solid transparent",
        paddingBottom: "3px",
        transition: "all 0.2s",
       }}
      >
    ...
    <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
     ...
     {currentMenu === "notice" && (
      <div style={{ height: "100%", overflowY: "auto", paddingRight: "4px", display: "flex", flexDirection: "column", gap: "20px" }}>
       <div>
        <h3 style={{ fontSize: "14px", color: "#9e7b5a", marginBottom: "8px", fontWeight: "bold" }}>소개</h3>
        <div
         style={{
          background: "rgba(240,228,210,0.28)",
          border: "1px solid rgba(200,178,148,0.28)",
          padding: "12px",
          fontSize: "12.5px",
          lineHeight: "1.6",
          color: "#6b5e50",
       ...
       <div>
        <h3 style={{ fontSize: "14px", color: "#9e7b5a", marginBottom: "8px", fontWeight: "bold" }}>연락처</h3>
        <div
         style={{
          background: "rgba(240,228,210,0.28)",
          border: "1px solid rgba(200,178,148,0.28)",
      ...
      <div style={{ height: "100%", overflowY: "auto", padding: "10px" }}>
       <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
        {samples.map((item) => (
         ...
         >
          <img src={item.src} alt={item.title} style={{ width: "100%", height: "120px", objectFit: "cover" }} />
          <p style={{ fontSize: "12px", textAlign: "center", marginTop: "5px", color: "#9e7b5a" }}>{item.title}</p>
         </div>
        ))}
       </div>
      </div>
     ...
     {currentMenu === "guest" && (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", height: "100%" }}>
       ...
       <div style={{ flex: 1, overflowY: "auto", paddingRight: "4px" }}>
        {messages.length === 0 ? (
         <p style={{ textAlign: "center", color: "#a89f91", fontSize: "13px", marginTop: "40px", fontStyle: "italic" }}>아직 메시지가 없습니다.</p>
        ) : (
         currentMessages.map((msg) => (
          <div
           key={msg.id}
           style={{ fontSize: "12.5px", borderBottom: "1px solid #f0e8dc", padding: "10px 0", display: "flex", gap: "8px", alignItems: "flex-start" }}
          >
           <Bud size={12} opacity={0.55} />
           <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
             ...
             <div style={{ display: "flex", gap: "5px" }}>
              <button
               onClick={(e) => {
                e.stopPropagation();
                onEditMessage(msg.docId, msg.password, msg.text);
               }}
               style={{ background: "none", border: "none", color: "#c4bcae", cursor: "pointer", fontSize: "10px" }}
              >
               수정
              </button>
              <button
               onClick={(e) => {
                e.stopPropagation();
                onDeleteMessage(msg.docId, msg.password);
               }}
               style={{ background: "none", border: "none", color: "#c4bcae", cursor: "pointer", fontSize: "10px" }}
              >
               삭제
       ...
       >
        ...
        <div style={{ display: "flex", gap: "5px" }}>
         ...
         />
         <button onClick={handleAddSubmit} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "14px", opacity: 0.6 }}>
          🖋️
         </button>
        </div>
       </div>
   ...
   {selectedImg &&
    createPortal(
     ...
     >
      <img
       src={selectedImg}
       style={{ maxWidth: "90%", maxHeight: "80%", WebkitTransform: "translate3d(0, 0, 0)", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}
      />
     </div>,
     document.body,
    )}
